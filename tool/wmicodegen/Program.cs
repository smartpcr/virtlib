            ...
            if ((m_modes & Params.generatesource) != 0)
            {
                ...
                if ((m_modes & Params.ns) != 0)
                {
                    ...
                    switch (m_fmt)
                    {
                        ...
                        case Format.GO:
                            Logger.Debug("Generating Go Source");
                            GoFormat fmt = new GoFormat(m_namespace, m_outputDir, recurse);
                            fmt.GenerateSources();
                            break;
                        case Format.RUST:
                            Logger.Debug("Generating Rust Source");
                            RustFormat rustFmt = new RustFormat(m_namespace, m_outputDir, recurse);
                            rustFmt.GenerateSources();
                            break;
                        default:
                            throw new NotImplementedException();
                    }
