...
/// 
    ...
    /// * `return_value` -  (u32)
    pub fn create(&self, interface_index: u32, interface_alias: &String, destination_prefix: &String, next_hop: &String, publish: u8, route_metric: u16, protocol: u16, compartment_id: u32, valid_lifetime: &String, preferred_lifetime: &String, policy_store: &String, address_family: u16, pass_thru: bool, cmdlet_output: &mut Vec<MSFT_NetRoute>) -> Result<(), WmiError> {
        ...
        args.push(MethodParameter { name: "CompartmentId".to_string(), value: compartment_id.into() });
...
/// 
    ...
    /// * `return_value` -  (u32)
    pub fn find(&self, interface_index: u32, local_ipaddress: &String, remote_ipaddress: &String, cmdlet_output: &mut Vec<CIM_ManagedElement>) -> Result<(), WmiError> {
        let mut args = Vec::new();
        args.push(MethodParameter { name: "InterfaceIndex".to_string(), value: interface_index.into() });
