...
pub struct MSFT_DtcAdvancedHostSettingTask {
}

impl MSFT_DtcAdvancedHostSettingTask {
...
/// 
    ...
    /// * `return_value` -  (u32)
    pub fn get(&self, name: &String, subkey: &String, cmdlet_output: &mut String) -> Result<(), WmiError> {
...
/// 
    ...
    /// * `return_value` -  (u32)
    pub fn set(&self, name: &String, subkey: &String, value: &String, type: &String) -> Result<(), WmiError> {
        let mut args = Vec::new();
        args.push(MethodParameter { name: "Name".to_string(), value: name.into() });
        args.push(MethodParameter { name: "Subkey".to_string(), value: subkey.into() });
