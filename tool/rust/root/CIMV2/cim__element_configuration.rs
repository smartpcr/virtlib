// Copyright 2019 (c) Microsoft Corporation.
// Licensed under the MIT license.

//
// Author:
//      Auto Generated on 12/5/2025 using wmigen
//      Source root.CIMV2
//////////////////////////////////////////////
use crate::wmi;
use crate::cim;


/// CIM_ElementConfiguration struct
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct CIM_ElementConfiguration {

/// 
    #[serde(rename = "Configuration")]
    pub configuration: Option<CIM_Configuration>,

/// 
    #[serde(rename = "Element")]
    pub element: Option<CIM_ManagedSystemElement>,
}

impl CIM_ElementConfiguration {
    /// Creates a new instance of the struct
    pub fn new() -> Self {
        Self {
            configuration: None,
            element: None,
        }
    }


    /// Sets the value of Configuration
    pub fn set_configuration(&mut self, value: CIM_Configuration) {
        self.configuration = Some(value);
    }

    /// Gets the value of Configuration
    pub fn get_configuration(&self) -> Option<&CIM_Configuration> {
        self.configuration.as_ref()
    }

    /// Sets the value of Element
    pub fn set_element(&mut self, value: CIM_ManagedSystemElement) {
        self.element = Some(value);
    }

    /// Gets the value of Element
    pub fn get_element(&self) -> Option<&CIM_ManagedSystemElement> {
        self.element.as_ref()
    }
}

