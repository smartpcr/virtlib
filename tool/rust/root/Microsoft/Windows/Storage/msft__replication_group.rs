...
/// 
    ...
    /// * `return_value` -  (u32)
    pub fn create_replica(&self, friendly_name: &String, target_storage_subsystem: MSFT_ReplicaPeer, target_group_object_id: &String, target_storage_pool_object_id: &String, recovery_point_objective: u32, replication_settings: MSFT_ReplicationSettings, sync_type: u16, run_as_job: bool, created_replica_peer: &mut MSFT_ReplicaPeer, created_storage_job: &mut MSFT_StorageJob, extended_status: &mut MSFT_StorageExtendedStatus) -> Result<(), WmiError> {
...
/// 
    ...
    /// * `extended_status` -  (MSFT_StorageExtendedStatus)
    /// * `return_value` -  (u32)
    pub fn set_replication_relationship(&self, operation: u16, target_group: MSFT_ReplicaPeer, source_storage_objects: &Vec<MSFT_StorageObject>, target_storage_objects: &Vec<MSFT_StorageObject>, sync_pairs: &Vec<MSFT_Synchronized>, run_as_job: bool, created_storage_job: &mut MSFT_StorageJob, extended_status: &mut MSFT_StorageExtendedStatus) -> Result<(), WmiError> {
        let mut args = Vec::new();
