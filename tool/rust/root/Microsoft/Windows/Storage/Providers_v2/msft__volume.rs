...
/// Indicates the deduplication mode of the volume.
    ...
    pub dedup_mode: Option<Volume_DedupMode>,
...
/// Denotes the type of the volume.
    ...
    pub drive_type: Option<Volume_DriveType>,
...
/// The underlying file system type of the volume.
    ...
    pub file_system_type: Option<Volume_FileSystemType>,
...
/// 2 - 'Unhealthy': The volume is not functioning, due to errors or failures. The volume needs immediate attention from an administrator.
    ...
    pub health_status: Option<Volume_HealthStatus>,
...
/// 0xD00F - 'Full Repair Needed': In Windows-based storage subsystems, this indicates full repair is needed.
    ...
    pub operational_status: Vec<Volume_OperationalStatus>,
...
/// Indicates the ReFS deduplication mode of the volume.
    ...
    pub re_fsdedup_mode: Option<Volume_ReFSDedupMode>,
...
impl MSFT_Volume {
    ...
    pub fn set_dedup_mode(&mut self, value: Volume_DedupMode) {
        self.dedup_mode = Some(value);
    }

    /// Gets the value of DedupMode
    pub fn get_dedup_mode(&self) -> Option<&Volume_DedupMode> {
    ...
    pub fn set_drive_type(&mut self, value: Volume_DriveType) {
        self.drive_type = Some(value);
    }

    /// Gets the value of DriveType
    pub fn get_drive_type(&self) -> Option<&Volume_DriveType> {
    ...
    pub fn set_file_system_type(&mut self, value: Volume_FileSystemType) {
        self.file_system_type = Some(value);
    }

    /// Gets the value of FileSystemType
    pub fn get_file_system_type(&self) -> Option<&Volume_FileSystemType> {
        self.file_system_type.as_ref()
    }

    /// Sets the value of HealthStatus
    pub fn set_health_status(&mut self, value: Volume_HealthStatus) {
        self.health_status = Some(value);
    }

    /// Gets the value of HealthStatus
    pub fn get_health_status(&self) -> Option<&Volume_HealthStatus> {
        self.health_status.as_ref()
    }

    /// Sets the value of OperationalStatus
    pub fn set_operational_status(&mut self, value: Vec<Volume_OperationalStatus>) {
        self.operational_status = value;
    }

    /// Gets the value of OperationalStatus
    pub fn get_operational_status(&self) -> &Vec<Volume_OperationalStatus> {
    ...
    pub fn set_re_fsdedup_mode(&mut self, value: Volume_ReFSDedupMode) {
        self.re_fsdedup_mode = Some(value);
    }

    /// Gets the value of ReFSDedupMode
    pub fn get_re_fsdedup_mode(&self) -> Option<&Volume_ReFSDedupMode> {
...
/// 
    ...
    /// * `return_value` -  (u32)
    pub fn format(&self, file_system: &String, file_system_label: &String, allocation_unit_size: u32, full: bool, force: bool, compress: bool, short_file_name_support: bool, set_integrity_streams: bool, use_large_frs: bool, disable_heat_gathering: bool, is_dax: bool, no_trim: bool, sha256_checksums: bool, formatted_volume: &mut MSFT_Volume, created_storage_job: &mut MSFT_StorageJob, extended_status: &mut MSFT_StorageExtendedStatus) -> Result<(), WmiError> {
...
/// 
    ...
    /// * `formatted_volume` -  (MSFT_Volume)
    /// * `return_value` -  (u32)
    pub fn format2(&self, file_system: &String, file_system_label: &String, allocation_unit_size: u32, full: bool, force: bool, compress: bool, short_file_name_support: bool, set_integrity_streams: bool, use_large_frs: bool, disable_heat_gathering: bool, is_dax: bool, no_trim: bool, sha256_checksums: bool, dev_drive: bool, formatted_volume: &mut MSFT_Volume, created_storage_job: &mut MSFT_StorageJob, extended_status: &mut MSFT_StorageExtendedStatus) -> Result<(), WmiError> {
        let mut args = Vec::new();
        args.push(MethodParameter { name: "FileSystem".to_string(), value: file_system.into() });
        args.push(MethodParameter { name: "FileSystemLabel".to_string(), value: file_system_label.into() });
        args.push(MethodParameter { name: "AllocationUnitSize".to_string(), value: allocation_unit_size.into() });
        ...
        let result = self.invoke_method("Format2", &args)?;
        let created_storage_job = result.get_value("CreatedStorageJob")?;
        let extended_status = result.get_value("ExtendedStatus")?;
        let formatted_volume = result.get_value("FormattedVolume")?;
        Ok(result.return_value)
...
/// 
    ...
    /// * `output` -  (u32)
    /// * `return_value` -  (u32)
    pub fn repair(&self, offline_scan_and_fix: bool, scan: bool, spot_fix: bool, detect_leaks: u32, scratch_file: &String, threads: u32, triage: bool, directory_ids: &Vec<u64>, salvage: u32, scratch_dir: &String, target_file: &String, target_dir: &String, output: &mut u32, created_storage_job: &mut MSFT_StorageJob, extended_status: &mut MSFT_StorageExtendedStatus) -> Result<(), WmiError> {
        let mut args = Vec::new();
...
/// 
    ...
    /// * `extended_status` -  (MSFT_StorageExtendedStatus)
    /// * `return_value` -  (u32)
    pub fn optimize(&self, re_trim: bool, analyze: bool, defrag: bool, slab_consolidate: bool, tier_optimize: bool, normal_priority: bool, created_storage_job: &mut MSFT_StorageJob, extended_status: &mut MSFT_StorageExtendedStatus) -> Result<(), WmiError> {
        let mut args = Vec::new();
...
/// 
    ...
    /// * `extended_status` -  (MSFT_StorageExtendedStatus)
    /// * `return_value` -  (u32)
    pub fn resize(&self, size: u64, created_storage_job: &mut MSFT_StorageJob, extended_status: &mut MSFT_StorageExtendedStatus) -> Result<(), WmiError> {
        let mut args = Vec::new();
