...
/// Media type of this storage tier
    ...
    pub media_type: Option<StorageTier_MediaType>,
...
impl MSFT_StorageTier {
    ...
    pub fn set_media_type(&mut self, value: StorageTier_MediaType) {
        self.media_type = Some(value);
    }

    /// Gets the value of MediaType
    pub fn get_media_type(&self) -> Option<&StorageTier_MediaType> {
...
/// This method allows the user to update or set various attributes on the storage tier. Note that not all parameters must be specified, and only those given will be updated.
    ...
    /// * `media_type` - Media type of this storage tier (StorageTier_MediaType)

    /// * `extended_status` - ExtendedStatus allows the storage provider to return extended (implementation specific) error information. (MSFT_StorageExtendedStatus)
    /// * `return_value` -  (u32)
    pub fn set_attributes(&self, media_type: StorageTier_MediaType, extended_status: &mut MSFT_StorageExtendedStatus) -> Result<(), WmiError> {
        let mut args = Vec::new();
...
/// 
    ...
    /// * `return_value` -  (u32)
    pub fn set_attributes2(&self, provisioning_type: u16, media_type: u16, fault_domain_awareness: u16, column_isolation: u16, resiliency_setting_name: &String, physical_disk_redundancy: u16, number_of_data_copies: u16, number_of_groups: u16, number_of_columns: u16, interleave: u64, extended_status: &mut String) -> Result<(), WmiError> {
        let mut args = Vec::new();
...
/// 
    ...
    /// * `extended_status` -  (String)
    /// * `return_value` -  (u32)
    pub fn set_attributes3(&self, provisioning_type: u16, allocation_unit_size: u64, media_type: u16, fault_domain_awareness: u16, column_isolation: u16, resiliency_setting_name: &String, usage: u16, physical_disk_redundancy: u16, number_of_data_copies: u16, number_of_groups: u16, number_of_columns: u16, interleave: u64, extended_status: &mut String) -> Result<(), WmiError> {
        let mut args = Vec::new();
        args.push(MethodParameter { name: "ProvisioningType".to_string(), value: provisioning_type.into() });
        ...
        let result = self.invoke_method("SetAttributes3", &args)?;
        let extended_status = result.get_value("ExtendedStatus")?;
        Ok(result.return_value)
...
/// This method returns the supported sizes for a new storage tier. These sizes can either be returned in an array of all supported sizes, through a min, max, and divisor, or both.
    ...
    /// * `extended_status` - ExtendedStatus allows the storage provider to return extended (implementation specific) error information. (MSFT_StorageExtendedStatus)
    /// * `return_value` -  (u32)
    /// * `supported_sizes` - This output parameter will contain an array of all of the supported sizes for the storage tier. This parameter may be NULL if the number of supported sizes is large, but is useful for storage tiers that only support a select number of tier sizes. (u64[])
    /// * `tier_size_divisor` - This parameter indicates the interval in which the supported sizes increment. For example: If the minimum supported size is 10 GB, and this parameter is 2 GB, then the supported sizes for this pool would be 10 GB, 12 GB, 14 GB, etc. until the maximum supported size is reached. (u64)
    /// * `tier_size_max` - This parameter denotes the maximum supported size that a tier created in this pool can be. (u64)
    /// * `tier_size_min` - This parameter denotes the minimum supported size that a tier created in this pool can be. (u64)
